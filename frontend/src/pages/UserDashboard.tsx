...
import { Upload, FileText, TrendingUp, Users, Clock, RefreshCw, ArrowRight } from 'lucide-react';
import type { Report } from '../lib/utils';

function StatCard({ icon: Icon, label, value, delay = 0, gradient }: any) {
  return (
    <div className="card p-5 fade-in-up hover:scale-[1.02] transition-all duration-300 cursor-default group"
...
export default function UserDashboard() {
  ...
  const avgAtt = history.length
    ? (history.reduce((a, r) => a + r.averageAttendance, 0) / history.length).toFixed(1) : '—';
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="flex items-start justify-between mb-8 fade-in-up">
        <div>
          ...
          <p className="text-sm mt-1" style={{ color: 'var(--foreground-3)' }}>Here's an overview of your attendance reports.</p>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          { icon: FileText,   label: 'Total Uploads',    value: history.length,  gradient: gradients[0] },
          { icon: Users,      label: 'Students Tracked', value: totalStudents,   gradient: gradients[1] },
          { icon: TrendingUp, label: 'Avg. Attendance',  value: avgAtt + '%',    gradient: gradients[2] },
          { icon: Clock,      label: 'Last Upload',
            value: history[0] ? new Date(history[0].uploadedAt).toLocaleDateString('en-IN') : '—',
            gradient: gradients[3] },
        ].map((c, i) => <StatCard key={c.label} {...c} delay={i * 60}/>)}
      </div>

      {/* Upload CTA */}
      <div className="rounded-2xl p-6 mb-8 fade-in-up relative overflow-hidden"
        style={{ background: 'linear-gradient(135deg, rgba(99,102,241,0.25) 0%, rgba(6,182,212,0.15) 100%)', border: '1px solid rgba(99,102,241,0.3)' }}>
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: 'radial-gradient(ellipse at 80% 50%, rgba(99,102,241,0.15) 0%, transparent 60%)' }}/>
        <div className="relative flex items-center justify-between gap-4">
          <div>
            <h2 className="text-lg font-bold mb-1" style={{ color: 'var(--foreground)' }}>Upload New Attendance Sheet</h2>
            <p className="text-sm" style={{ color: 'var(--foreground-2)' }}>
              Process Excel sheets, visualize data, and generate defaulter letters.
            </p>
          </div>
      ...
      <div className="card overflow-hidden fade-in-up">
        <div className="px-6 py-4 flex items-center justify-between" style={{ borderBottom: '1px solid var(--border)' }}>
          <h2 className="font-bold" style={{ color: 'var(--foreground)' }}>Recent Uploads</h2>
          {history.length > 0 && (
        ...
        {!fetching && !authLoading && !fetchErr && history.length === 0 && (
          <div className="p-14 text-center">
            ...
            <p className="font-semibold" style={{ color: 'var(--foreground-2)' }}>No reports yet</p>
            <p className="text-sm mt-1" style={{ color: 'var(--foreground-3)' }}>Upload your first sheet to get started.</p>
          </div>
        )}
        {!fetching && !authLoading && !fetchErr && history.length > 0 && (
          <table className="w-full text-sm">
            <thead>
              <tr style={{ background: 'var(--surface)', borderBottom: '1px solid var(--border)', color: 'var(--foreground-3)' }}>
                {['File Name','Students','Avg. Att.','Uploaded On','Action'].map(h => (
            ...
            <tbody>
              {history.map(r => (
                <tr key={r._id} className="transition-all duration-200 group"
                  ...
                  <td className="px-6 py-3.5">
                    <span className="font-bold text-sm" style={{ color: r.averageAttendance < 75 ? '#fb7185' : '#34d399' }}>
