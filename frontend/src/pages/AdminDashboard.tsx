...
export default function AdminDashboard() {
  ...
  const totalStudents = history.reduce(
    (a: number, r: any) => a + r.totalStudents,
    0,
  );
  const avgAtt = history.length
    ? (
        history.reduce((a: number, r: any) => a + r.averageAttendance, 0) /
        history.length
      ).toFixed(1)
    : "—";
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
        {stats.map(({ icon: Icon, label, value, grad }, i) => (
          <div
            key={label}
            className="card p-5 hover:scale-[1.02] transition-all duration-300 fade-in-up"
            ...
            <tbody>
              {history.map((r: any) => (
                ...
                >
                  ...
                  <td
                    className="px-6 py-3.5 text-xs max-w-xs truncate"
                    style={{ color: "var(--foreground-3)" }}
                  >
                    {r.dateRange || "—"}
                  </td>
